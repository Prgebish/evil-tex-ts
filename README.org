...
*Important:* The variable ~evil-tex-ts-preferred-inline-math~ controls which format is used when toggling math modes (=mtm=) and surrounding with inline math (=m=). Set it to ='dollar= for =$...$= or ='paren= for =\(...\)=.
