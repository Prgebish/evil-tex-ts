...
- =die= (delete inner environment)
  ...
  Point can be anywhere inside the environment:
...
Jump to the next/previous section heading (chapter, section, subsection, etc.).
...
* Acknowledgements
- [[https://github.com/iyefrat/evil-tex][evil-tex]] for the original concept and features.
...
| ~x~ | *prompt*  | ~m~ | multline    |
| ~e~ | equation  | ~c~ | cases       |
| ~E~ | equation* | ~z~ | tikzpicture |
| ~a~ | align     | ~f~ | figure      |
| ~A~ | align*    | ~i~ | itemize     |
| ~g~ | gather    | ~I~ | enumerate   |

*** Delimiter Keymap (~d~)
| Key | Simple | Sized (~p~/~s~/~c~) |
|-----+--------+------------------|
| ~P~ | ~()~   | ~\left(\right)~  |
| ~S~ | ~[]~   | ~\left[\right]~  |
| ~C~ | ~\{\}~ | ~\left\{\right\}~ |
| ~V~ | ~\vert\vert~ | ~\left\vert\right\vert~ |
